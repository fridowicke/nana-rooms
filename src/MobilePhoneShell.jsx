...
const ROOMS = [
  ...
  { id: 'pardis', title: 'PARDIS', file: 'PARDIS WEB.glb', note: 'room 05' },
  { id: 'kaori', title: 'KAORI', file: 'KAORI WEB.glb', note: 'room 06' },
  { id: 'rei', title: 'REI', file: 'REI WEB.glb', note: 'room 07' },
  { id: 'yuria', title: 'YURIA', file: 'YURIA WEB.glb', note: 'room 08' },
  { id: 'momoco', title: 'MOMOCO', file: 'MOMOCO WEB.glb', note: 'room 09' },
  { id: 'kumo-muki', title: 'KUMO&MUKI', file: 'KUMO&MUKI WEB.glb', note: 'room 10' },
  { id: 'mimi', title: 'MIMI', file: 'MIMI WEB.glb', note: 'room 11' },
]
...
function MobileNotice() {
  return (
    <section className="mobile-notice" aria-label="Desktop notice">
      <img src="assets/home-welcome.gif" alt="Welcome to my page" />
...
function RoomTicker() {
  return (
    <div className="room-ticker" aria-hidden="true">
      <span>clean my room +++++ clean my room +++++ clean my room +++++ clean my room +++++</span>
    </div>
  )
}
    ...
    <div className="stacked-app">
      ...
      <div className="room-jump-row">
        {ROOMS.map((room, index) => (
          <button type="button" onClick={() => openRoom(index)} key={room.id}>
  ...
  return (
    <div className="artworks-app">
      ...
      <div className="mini-room-grid">
        {ROOMS.map((room, index) => <button type="button" onClick={() => openRoom(index)} key={room.id}>{room.note}</button>)}
      </div>
...
function RoomApp({ index, openRoom }) {
  ...
  return (
    <div className="stacked-app">
      <RoomTicker />
      <ThreeViewer url={`rooms/${room.file}`} title={`${room.note} / ${room.title}`} />
      <div className="room-jump-row">
...
export default function App() {
  ...
  const openRoom = (index) => {
    setRoomIndex(index)
    openApp({ id: 'room', title: `${ROOMS[index].note} / ${ROOMS[index].title}` })
  }
